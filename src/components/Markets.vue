<template>
  <div class="container justify-content-center" id="coin">
    ...
    <section v-if="errored">
      <p>
        We are sorry, we are unable to retrieve this information at this time.
        Please retry later
      </p>
    </section>
    ...
    <section v-else>
      ...
      <div>
        <b-button v-b-toggle.collapse-1 class="m-2" variant="dark"
          ><b-icon icon="sliders"></b-icon> Filters</b-button
        >
        <b-collapse id="collapse-1" class="mt-2">
          <b-form-group label="Nbs per page" class="d-inline p-2">
            <b-form-radio v-model="per_page" name="some-radios" value="50"
              >50</b-form-radio
            >
            <b-form-radio v-model="per_page" name="some-radios" value="100"
              >100</b-form-radio
            >
            <b-form-radio v-model="per_page" name="some-radios" value="250"
              >250</b-form-radio
            >
          </b-form-group>
          <b-form-group label="Currency" class="d-inline p-2">
            <b-form-radio v-model="currency" name="some-radios" value="usd"
              >$</b-form-radio
            >
            <b-form-radio v-model="currency" name="some-radios" value="eur"
              >€</b-form-radio
            >
            <b-form-radio v-model="currency" name="some-radios" value="jpy"
              >¥</b-form-radio
            >
          </b-form-group>
          <b-pagination
      ...
      <v-table :data="infos" class="table table-striped table-dark">
        <thead slot="head">
          <!---Balise "v-th" pour indiquer les colonnes actives--->
          <v-th scope="col" sortKey="market_cap_rank" defaultSort="asc"
            >Rank</v-th
          >
          <th scope="col">Logo</th>
          <v-th scope="col" sortKey="name">Name</v-th>
          <v-th scope="col" sortKey="current_price">Price</v-th>
          <v-th scope="col" sortKey="price_change_percentage_24h_in_currency"
            >Last(24h)</v-th
          >
          <v-th scope="col" sortKey="price_change_percentage_7d_in_currency"
            >Last(7d)</v-th
          >
        ...
        <tbody slot="body" slot-scope="{ displayData }">
          <tr v-for="info in displayData" :key="info.guid">
            ...
            >
              {{
                info.price_change_percentage_24h_in_currency | currencydecimal
              }}
              %
            </td>
            <td
              :class="changeColor(info.price_change_percentage_7d_in_currency)"
            >
              {{
                info.price_change_percentage_7d_in_currency | currencydecimal
              }}
              %
            </td>
